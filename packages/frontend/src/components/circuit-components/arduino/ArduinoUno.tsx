  ...
  return (
    ...
    >
      ...
      <Group scaleX={sx} scaleY={sy}>
        ...
        {/* Soldered contacts and vias */}
        {[
          [210, 110], [210, 125], [210, 140],
          [225, 110], [225, 125], [225, 140],
          [240, 110], [240, 125], [240, 140]
        ].map(([cx, cy], i) => (
          <Circle key={`via1-${i}`} x={cx} y={cy} radius={3} fill="#cca43b" stroke="#333" strokeWidth={0.5} opacity={0.8} />
        ))}
        {[
          [682, 300], [682, 315], [682, 330],
          [697, 300], [697, 315], [697, 330]
        ].map(([cx, cy], i) => (
          <Circle key={`via2-${i}`} x={cx} y={cy} radius={3.5} fill="#a1a1a1" stroke="#333" strokeWidth={0.5} />
        ))}
        {[
          [48, 310], [180, 460], [280, 390], [340, 310], [600, 110], [700, 80]
        ].map(([cx, cy], i) => (
        ...
        {[
          [188, 22, 24], [132, 483, 32], [685, 192, 32], [685, 442, 32]
        ].map(([cx, cy, d], i) => (
      ...
      {Object.values(component.pins).map(pin => {
        ...
        // Visual properties for 4x4 squares (in 200x140 space)
        const w = 4.6;
        const h = 4.6;

        let fillCol = '#171717'; // default core hole
        let glowCol = 'transparent';
        if (isHigh && (pin.type === 'digital' || pin.type === 'PWM' || pin.type === 'power')) {
          fillCol = '#22d3ee'; // bright cyan glow for HIGH
          glowCol = '#22d3ee';
        }
        ...
        return (
          ...
          >
            ...
            {/* Konva Tooltip (on hover) */}
            {isHovered && !isNC && (
              <Label x={w/2 + 5} y={h/2 - 15} opacity={1} listening={false}>
                <Tag fill="#1f2937" cornerRadius={2} pointerDirection="down" pointerWidth={4} pointerHeight={4} />
                <Text
